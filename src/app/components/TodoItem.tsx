...
const convertDate = (num: string) => {
    num = num.toString();
    if (num.length > 0) {
        let newDate = num.toString();
        if (num.length <= 10) newDate = (+num * 1000).toString();
        return new Intl.DateTimeFormat('en-GB', { day: '2-digit', month: '2-digit', year: '2-digit' }).format(+newDate)
    }
}

const TodoItem: React.FC<Props> = ({ todo, setTodoList }) => {
    // what to do:
